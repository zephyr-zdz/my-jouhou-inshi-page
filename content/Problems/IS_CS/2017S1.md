...
3. 我们固定字母表 $\Sigma$ 为 $\Sigma = \{a, b\}$。对于问题 (1) 中的语言 $L_1$，给出一个不超过 5 个状态的确定性有限自动机 (DFA) $\mathcal{A_2}$，使得：$\mathcal{L}(\mathcal{A_2}) = \Sigma^* \setminus L_1$。这里 $\Sigma^* \setminus L_1$ 表示 $\Sigma^*$ 中 $L_1$ 的补集。
